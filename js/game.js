    ...
    theloarian: [
        ...
        },
        {
            ...
            special: {
                ...
                value: 20,
                description:
                    "Channels cosmic energy to attack and restore power",
            },
        },
        {
            id: "lyrrija",
...
function updateBattleUI() {
    ...
    const playerImagePath = gameState.player.image;
    playerPortrait.style.backgroundImage = `url('${playerImagePath}')`;
    ...
    updateEnergyBar(
        "player",
        gameState.player.currentEnergy,
    ...
    const enemyImagePath = gameState.enemy.image;
    enemyPortrait.style.backgroundImage = `url('${enemyImagePath}')`;
    enemyPortrait.style.backgroundSize = 'cover';
    enemyPortrait.style.backgroundPosition = 'center';
    enemyPortrait.style.backgroundRepeat = 'no-repeat';
    ...
    updateEnergyBar(
        "enemy",
        gameState.enemy.currentEnergy,
        gameState.enemy.maxEnergy
    );
...
function startBattle() {
    ...
    const enemyFaction =
        gameState.selectedFaction === "theloarian" ? "meduar" : "theloarian";
...
function updateHealthBar(target, current, max) {
    ...
    document.getElementById(`${target}HealthText`).textContent = `${Math.max(
        0,
        current
    )}/${max}`;
}
...
function updateEnergyBar(target, current, max) {
    ...
    document.getElementById(`${target}EnergyText`).textContent = `${Math.max(
        0,
        current
    )}/${max}`;
}
...
function updateActionButtons() {
    ...
    buttons.forEach((btn) => {
        ...
        btn.disabled =
            ...
            gameState.player.currentEnergy < energyCost;
    });
}
    ...
    switch (special.effect) {
        ...
        case "energy": {
            ...
            attacker.currentEnergy = Math.min(
                attacker.maxEnergy,
                attacker.currentEnergy + special.value
            );
            addBattleMessage(
        ...
        case "drain": {
            ...
            attacker.currentHealth = Math.min(
                attacker.maxHealth,
                attacker.currentHealth + special.value
            );
            addBattleMessage(
...
function enemyTurn() {
    ...
    if (
        healthPercentage < 30 &&
        enemy.currentEnergy >= 20 &&
        enemy.special.effect === "heal"
    ) {
        performHeal(enemy);
...
function endBattle(playerWon) {
    ...
    <p><strong>Your Champion:</strong> ${gameState.player.name} (${
        gameState.player.role
    })</p>
    <p><strong>Final HP:</strong> ${Math.max(
        0,
        gameState.player.currentHealth
    )}/${gameState.player.maxHealth}</p>
    <p><strong>Enemy:</strong> ${gameState.enemy.name} (${
        gameState.enemy.role
    })</p>
    <p><strong>Enemy HP:</strong> ${Math.max(
        0,
        gameState.enemy.currentHealth
    )}/${gameState.enemy.maxHealth}</p>
  `;
